...
const handler = async (event) => {
  const {
    telephone, address, email, eventInfo,
  } = JSON.parse(event.body);
  logger.debug(`In the update contact handler with: telephone ${telephone} address ${address} Email ${email} and eventInfo ${eventInfo}`);
  eventInfo.eventDate = dateFormat(new Date(), 'isoDateTime');

  try {
    const response = await updateContact(telephone, address, email, eventInfo, logger);
    return {
      statusCode: 201,
