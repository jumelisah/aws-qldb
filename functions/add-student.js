...
const handler = async (event) => {
  ...
  logger.debug(`In the create licence handler with: first name ${firstName} last name ${lastName} email ${email} telephone ${telephone} and address ${address}`);
  ...
  try {
    ...
    const response = await createLicence(
      firstName, lastName, email, telephone, address, eventInfo, logger,
    );
