...
const { updateStudentData } = require('./helper/licence');
...
const handler = async (event) => {
  ...
  logger.debug(`In the update licence handler with studentId ${data.studentId}`);

  try {
    const response = await updateStudentData(data, logger);
    return {
      statusCode: 200,
